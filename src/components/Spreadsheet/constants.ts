...
export function getCellIndex(cellId: string): number {
  const column = COLUMN_LABELS.indexOf(cellId[0])
  const row = Number(cellId.slice(1)) - 1
  return row * COLUMN_COUNT + column
}

export function getCellElementId(cellId: string): string {
  return `spreadsheet-cell-${cellId}`
}
